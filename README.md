...
Fail pull requests that add abandoned, non-New-Architecture, or npm-deprecated React Native dependencies â€” with inline annotations and a policy-as-code allowlist.
...
Unmaintained dependencies are a perennial top-5 pain in React Native surveys, and the cost keeps climbing now that the New Architecture is the default: a package that looked fine when it was added quietly stops getting releases, never gains New Architecture support, or gets deprecated on npm â€” and you find out during an upgrade, months after the PR that introduced it merged.

Lookup tools exist (the React Native Directory tells you a package's status if you go ask). What's been missing is **enforcement**: the check that runs on every PR, fails when someone adds a dying dependency, and points at the exact line â€” so the conversation happens at review time, when swapping the package costs five minutes instead of a quarter.
...
- `changed-only` requires `actions/checkout` with `fetch-depth: 0` â€” the diff is computed against the merge-base of the PR branch and `base`, same as GitHub's three-dot compare. Without history the run fails with an actionable message telling you exactly that.
- The default `token` (the workflow's own `GITHUB_TOKEN`) is enough; it's only used to read public repo metadata (archived state, last push). Without any token those fields degrade to `unknown` with a warning â€” the run never fails because of rate limits.
...
**Exit codes** (stable contract â€” CI depends on it):
...
Only `dependencies` are checked â€” `devDependencies` never ship in your app.
...
**How the data is gathered.** Each dependency is enriched in parallel from the npm registry (publish time, deprecation, `codegenConfig`), the [React Native Directory](https://reactnative.directory) (New Architecture support, unmaintained flag, GitHub URL), and the GitHub API (archived, last push â€” token optional). Results are cached in `.rn-doctor-cache.json` for 24 hours (add it to `.gitignore`; `--no-cache` bypasses).

**New Architecture verdicts** are tiered honestly: directory says supported â†’ pass; directory says unsupported â†’ `newArchitecture` fires; unknown but the package ships `codegenConfig` â†’ pass with an informational note; unknown otherwise â†’ `newArchUnknown` fires (a warning by default â€” missing data is not the same as a dead package).
...
- Suppressed findings still appear in output (marked as allowed) â€” nothing disappears silently.
...
- **Pretty** (default): one block per finding â€” severity badge, message, allow-reason if suppressed, and an evidence link (npm/directory/GitHub) so you can verify every claim.
- **`--json`**: stable-ordered, timestamp-free (`version: 1`) â€” safe to snapshot or post-process.
...
**Does it check devDependencies?** No â€” they don't ship in your app. Scope decisions live in the policy file, not flags.
...
**Does it fix anything?** No â€” it tells you what's wrong and what to do (allowlist entry, `npx expo install --fix`, Renovate), then gets out of the way.

**What if the RN Directory is wrong about a package?** Every finding carries an evidence link so you can check, and a false positive costs one allowlist line. File a correction with the directory â€” everyone benefits.

**Why is `yaml` the only runtime dependency?** The policy file and `pnpm-workspace.yaml` are YAML, and a spec-compliant YAML parser is not a weekend project â€” this one is worth its weight. Everything else (HTTP, caching, git diffing, glob matching, SARIF, annotations) is hand-rolled on Node built-ins; less supply chain in a tool whose whole job is judging dependencies.

## Contributing

Issues and PRs welcome â€” see [CONTRIBUTING.md](CONTRIBUTING.md). The project follows a phased roadmap; out-of-scope items (security auditing, auto-fixing, a hosted service) are listed there.

## License

[MIT](LICENSE) Â© Amrith Vengalath
